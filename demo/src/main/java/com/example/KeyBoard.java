...
public class KeyBoard {
    ...
    //监听按键松开
    public static void AddKeyRelease(JComponent jc, String key, String actionName, Runnable action) {
    InputMap inputMap = jc.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW);
    ActionMap actionMap = jc.getActionMap();
    
    inputMap.put(KeyStroke.getKeyStroke("released " + key), actionName);
    actionMap.put(actionName, new AbstractAction() {
        @Override
        public void actionPerformed(ActionEvent e) {
            action.run();
